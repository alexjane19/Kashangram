...
public class UserProfileAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public UserProfileAdapter(Context context, JSONArray arr) {
        this.context = context;
        this.cellSize = Utils.getScreenWidth(context) / 3;
        this.photos = arr;//Arrays.asList(context.getResources().getStringArray(R.array.user_photos));
    }
    ...
    private void bindPhoto(final PhotoViewHolder holder, int position) {
        ...
        String name_photo = "";
        try {
            obj = (JSONObject) photos.get(position);
            name_photo = String.valueOf(obj.get(PHOTO_ID)) + ".jpg";
        } catch (JSONException e) {
            e.printStackTrace();
        }
        Picasso.with(context)
                .load(URL_UPLOADED_IMAGE + name_photo)
    ...
    public int getItemCount() {
        return photos.length(); //photos.size();
    }
